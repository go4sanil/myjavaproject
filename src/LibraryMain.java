	...
	public static void main(String[] args) {
		...
		do {
			...
			for (int i = 0; i < bookDetails.length; i++) {
				...
				else if (requiredBookName.equals(bookDetails[i].bookName) && bookAuthor.equals(bookDetails[i].authorName) && bookDetails[i].isBookIssued)
				{
					...
					else if(!(bookAuthor.equals(bookDetails[i].authorName)) && !(bookAuthor.equals(bookDetails[i].authorName))) {
					counter++;
					if ((counter) == (bookDetails.length)) {
		...
		System.out.println("The maximum issuing limit reached");
	}
}
